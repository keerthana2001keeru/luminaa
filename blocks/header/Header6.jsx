...
function Header6(props) {
  ...
  const removeMenu = () => {
    dispatch({type: "REMOVE_SUB_MENU"})
   }

  var header04 = props?.header04?.blocks?.TH00150C06B01?.elements;
  
  var siteLogo=process?.env?.siteIdentity?.headerLogo ? process?.env?.siteIdentity?.headerLogo : process?.env?.siteIdentity?.siteLogo;

  if (siteLogo == "") {
  siteLogo = process.env.basePath + '/assets/img/logo-lumina.png';
}
    ...
    >
      ...
      <div className="header-logo">
        <Link legacyBehavior href="/">
          <a>
            {/* <img alt="image" className="img-fluid" src={header04?.logo?.logoImage?.value} /> */}
            <img
                alt="image"
                className={classes.sidebarLogo + " img-fluid"}
                src={siteLogo }
      ...
      <div className={`main-menu ${state.isSidebarOpen ? "show-menu" : ""}`}>
        ...
        {header04?.navMenu?.categoriesMenu?.value?.map((item, index) => (
           <li className={`menu-item-has-children ${
            currentRoute == item.link ? "active" : ""
              }`} key={index}>

              <Link legacyBehavior href={item?.link || "#"}><a className={`drop-down ${
                state.activeMenu === item?.name ? "active" : ""
              }`}
              onClick={() => {toggleMenu(item?.name)
                toggleSidebar
                  ...
                  {item.children.length > 0 &&
                   <span className="mobile-hidden"><BiChevronDown/></span>
                   }
                </a>
                </Link>

            {item.children.length > 0 &&<i
            ...
            <ul
              className={`sub-menu ${
                state.activeMenu === item?.name ? "d-block" : ""
              }`}
            >

             {item?.children?.map((element, index) => (
              <li key={index}>
                <Link legacyBehavior href={element?.link || ""}>
                  <a>{element?.name}</a>
                </Link>
                ...
                <i
                  className={`bi bi-plus dropdown-icon ${
                    state.activeSubMenu === element?.name ? "active" : ""
                  }`}
                  onClick={() => toggleSubMenu(element?.name)}
                />
                )}
                {element?.children?.length > 0 && (
                <ul
                  className={`sub-menu ${
                    state.activeSubMenu === element?.name ? "d-block" : ""
                  }`}
                >

                 {element?.children?.map((data, index) => (
                  <li key={index}>
                    <Link legacyBehavior  href={data?.link || "#"}>
                      <a  className={currentRoute === data?.link ? "active" : ""}>
                        {data?.name}</a>
                    </Link>
                  </li>
                  ))} 
          ...
          <form className="mobile-menu-form">
            <div className="hotline pt-30">
              ...
              <div className="hotline-info">
                <span>{header04?.offcanvas?.phoneTitle?.value}</span>
                <h6>
                <Link href={header04?.offcanvas?.phoneLink?.value || "#"} legacyBehavior><a>{header04?.offcanvas?.phoneNumber?.value}</a></Link>
                </h6>
              </div>
            </div>
            <div className="email pt-20 d-flex align-items-center">
              ...
              <div className="email-info">
                <span>{header04?.offcanvas?.emailInfoTitle?.value}</span>
                <h6>
                <Link href={header04?.offcanvas.emailLink.value || "#"}  legacyBehavior><a>{header04?.offcanvas?.emailID?.value}</a></Link>
                </h6>
              </div>
            </div>
          </form>
          {header04?.navbarBtn?.btnText?.value ? 
          <div className="header-btn5">
            <Link legacyBehavior href={header04?.navbarBtn?.btnLink?.value || "#"}>
              <a className="primary-btn3">{header04?.navbarBtn?.btnText?.value}</a>
            </Link>
          </div>
           : ""} 
        </div>
      ...
      <div className="nav-right d-flex jsutify-content-end align-items-center">
        ...
        <div className="header-contact d-xl-block d-none">
          <span>
            <img src={header04?.headerContact?.contactIcon?.value} alt="" />
            {/* <img src="assets/img/home-6/phone.svg" alt="" /> */}
            {header04?.headerContact?.heading?.value}
          </span>
          <h6>
            <Link href={`tel:${header04?.headerContact?.itemLink?.value}`}>{header04?.headerContact?.contactItem?.value}</Link>
          </h6>
        </div>
        ...
        <div className="header-btn d-sm-flex d-none">
          <Link legacyBehavior href={header04?.navbarBtn?.btnLink?.value || "#"}>
            <a>{header04?.navbarBtn?.btnText?.value}</a>
          </Link>
        </div>
         : ""} 
        <div
