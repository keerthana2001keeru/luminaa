...
function Header4(props) {
  ...
  var header = props?.header02?.blocks?.TH00150C05B01?.elements;
  // console.log(header);
  ...
  {siteLogo &&
      ...
      <div className={`main-menu ${state.isSidebarOpen ? "show-menu" : ""}`}>
        ...
        {header?.navMenu?.categoriesMenu?.value?.map((item, index) => (
          ...
          <form className="mobile-menu-form">
            <div className="hotline pt-30">
              ...
              <div className="hotline-info">
                <span>{header?.offcanvas?.phoneTitle?.value}</span>
                <h6>
                <Link href={header?.offcanvas?.phoneLink?.value || "#"} legacyBehavior><a>{header?.offcanvas?.phoneNumber?.value}</a></Link>
                </h6>
              </div>
            </div>
            <div className="email pt-20 d-flex align-items-center">
              ...
              <div className="email-info">
                <span>{header?.offcanvas?.emailInfoTitle?.value}</span>
                <h6>
                <Link href={header?.offcanvas.emailLink.value || "#"}  legacyBehavior><a>{header?.offcanvas?.emailID?.value}</a></Link>
                
                </h6>
              </div>
            </div>
          </form>
          {header?.navbarBtn?.btnText?.value ?
            <div className="header-btn5">
              <Link legacyBehavior href={header?.navbarBtn?.btnLink?.value || "#"}>
                <a className="primary-btn3">{header?.navbarBtn?.btnText?.value}</a>
              </Link>
            </div>
              : ""} 
        </div>
      ...
      {header?.navbarBtn?.btnText?.value ?
        <div className="header-btn d-sm-flex d-none">
          <Link legacyBehavior href={header?.navbarBtn?.btnLink?.value || "#"}>
            <a>{header?.navbarBtn?.btnText?.value}</a>
          </Link>
        </div>
        : ""} 
